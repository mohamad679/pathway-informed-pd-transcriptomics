...
from models.logistic_baseline import DEFAULT_SEEDS, evaluate_logistic_regression_cv


PROCESSED_DIR = ROOT / "data" / "processed"
RESULTS_PATH = ROOT / "results" / "development" / "logistic_baseline_cv.csv"
REPORT_PATH = ROOT / "docs" / "phase2_logistic_baseline.md"
DECISION_LOG_PATH = ROOT / "docs" / "decision_log.md"
METRIC_NAMES = ("auroc", "auprc", "balanced_accuracy", "brier", "ece")

DECISION_LOG_ENTRY = """## 2026-07-10 — Phase 2 development-only Logistic Regression baseline
...
def write_report(rows: list[dict[str, float | int | str]], summary: dict[str, float]) -> None:
    ...
    report = f"""# Phase 2 Logistic Regression development-only baseline
...
def main() -> int:
    ...
    rows = evaluate_logistic_regression_cv(X, y, folds, seeds=DEFAULT_SEEDS)
    ...
    print("Development-only Logistic Regression cross-validation")
